#!/usr/bin/env python3
"""Replay parallel-v5.2.1 hybrid games without exact stochastic reruns."""
...
SUITE = "five-star-parallel-v521-final"
PROFILES = {
...
BLACK_COMPONENTS = {
    "serial": (4, "black-v521-serial-1w-control",
               "5.2.1-certificate-dependency-widening-1w-5s-control"),
    "parallel": (3, "black-v521-parallel-root-8w",
                 "5.2.2-certificate-dependency-root-parallel-8w-5s"),
}
...
def validate_header(header: dict, schedule: dict,
                    forbidden: bool, mode: str) -> int:
    assert header["schemaVersion"] >= 5
    assert header["suite"] == SUITE
    assert header["newProfile"]["version"] == PROFILES[mode]
...
def validate_new_step(step: dict, side: int, master: int,
                      opening_id: int, ply: int, mode: str) -> dict[str, int]:
    expected = WHITE_COMPONENT if side == -1 else BLACK_COMPONENTS[mode]
    ...
    if side == -1:
        ...
        assert cap == 1 and workers == 0 and jobs == 0
    else:
        assert cap == 8
    ...
    return {
        ...
        "blackParallelSteps": int(side == 1 and workers > 0),
        "blackWorkerLaunches": workers if side == 1 else 0,
        "blackParallelJobs": jobs if side == 1 else 0,
    }


def validate_game(library, game: dict, entries: dict[int, dict],
                  forbidden: bool, master: int, mode: str) -> dict[str, int]:
    ...
    totals = {"randomSelections": 0, "budgetExhaustions": 0,
              ...
              "blackParallelSteps": 0, "blackWorkerLaunches": 0,
              "blackParallelJobs": 0}
        ...
        if ply >= len(prefix):
            ...
            if side == game["newColor"]:
                audited = validate_new_step(
                    step, side, master, opening_id, ply, mode)
                for key, value in audited.items():
                    totals[key] += value
            else:
                assert step["engine"] == "four-star"
                assert step.get("proofWorkerCap", 0) == 0
...
def replay_cell(library, result_path: Path, schedule_path: Path,
                forbidden: bool, mode: str) -> dict:
    header, games = load_jsonl(result_path)
    schedule = json.loads(schedule_path.read_text(encoding="utf-8"))
    entries = schedule_entries(schedule)
    master = validate_header(header, schedule, forbidden, mode)
    ...
    totals = {"moves": 0, "randomSelections": 0, "budgetExhaustions": 0,
              ...
              "blackParallelJobs": 0}
    for game in games:
        audited = validate_game(library, game, entries, forbidden, master, mode)
        for key, value in audited.items():
            totals[key] += value
    if mode == "serial":
        assert totals["blackWorkerLaunches"] == 0
    else:
        assert totals["blackParallelSteps"] > 0
        assert totals["blackWorkerLaunches"] > 0
    return {
        "result": str(result_path), "resultSha256": sha256(result_path),
        "schedule": str(schedule_path), "scheduleSha256": sha256(schedule_path),
        "forbiddenBlack": forbidden, "mode": mode,
...
def main() -> None:
    ...
    summary = {
        ...
        "blackParallelSteps": sum(cell["blackParallelSteps"] for cell in cells),
        "blackWorkerLaunches": sum(cell["blackWorkerLaunches"] for cell in cells),
        "cells": cells,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    print(f"parallel-v521 replay: pass; games={summary['games']} "
          f"moves={summary['moves']} black-parallel={summary['blackParallelSteps']}")
