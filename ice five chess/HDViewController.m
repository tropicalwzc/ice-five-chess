...
- (void)viewDidLoad {
    ...
    if(rander==0)
     ...
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(try_next_step) name:@"trynow" object:nil];
    thread_num=2;
...
{
    if(val==1){
        [chess_map[x][y] setImage:[UIImage imageNamed:@"hdblack_chess_pic_j"] forState:UIControlStateNormal];
    }
    if(val==-1){
        [chess_map[x][y] setImage:[UIImage imageNamed:@"hdwhite_chess_pic_j"] forState:UIControlStateNormal];
    }
    if(val==0){
        [chess_map[x][y] setImage:[UIImage imageNamed:@"hdaimed_pic"] forState:UIControlStateNormal];
    }
...
-(void) paint_focus_chess_map_with_x:(long)x y:(long)y val:(int)val
{
    if(x>0&&map_state[x-1][y]==0)
        [chess_map[x-1][y] setImage:[UIImage imageNamed:@"hd_righter"] forState:UIControlStateNormal];
    else if(x<14&&map_state[x+1][y]==0)
        [chess_map[x+1][y] setImage:[UIImage imageNamed:@"hd_lefter"] forState:UIControlStateNormal];
    else if(y<14&&map_state[x][y+1]==0)
        [chess_map[x][y+1] setImage:[UIImage imageNamed:@"hd_upper"] forState:UIControlStateNormal];
    else if(y>0&&map_state[x][y-1]==0)
        [chess_map[x][y-1] setImage:[UIImage imageNamed:@"hd_downer"] forState:UIControlStateNormal];
    else{
        if(val==1){
            [chess_map[x][y] setImage:[UIImage imageNamed:@"hdblack_chess_pic_l"] forState:UIControlStateNormal];
        }
        if(val==-1){
            [chess_map[x][y] setImage:[UIImage imageNamed:@"hdwhite_chess_pic_l"] forState:UIControlStateNormal];
        }
    }
}
...
-(void) flush_chess_map_according_to:(int[15][15])row_map
{
    ...
    if(ice_fiver!=nil)
    {
      ...
      //  else lcolor=[ice_whiter get_last_pos_return_color:pos];
...
-(void) set_base_block:(int)val x:(int)x y:(int)y back_color:(UIColor*)background_color title_color:(UIColor*)title_color
{
    ...
    ScreenWidth=(int)Screensize.size.width;
    ScreenHeight=(int)Screensize.size.height;
    float smaller = ScreenWidth<ScreenHeight?ScreenWidth:ScreenHeight;
    float larger = ScreenWidth>ScreenHeight?ScreenWidth:ScreenHeight;
    float per = smaller/20;
    float basex,basey;
    if(ScreenHeight<ScreenWidth)
    {
        per=(ScreenHeight-150)/16;
        basex=(ScreenWidth-per*15)/2;
        basey = 120;
    }
    else{
        per=(ScreenWidth)/16;
        basex=(ScreenWidth-per*15)/2;
        basey = 80+ (ScreenHeight-150-per*15)/2;
    }
    CGRect position=CGRectMake(0, 0, 0, 0);
    position=CGRectMake(basex+x*per, basey+y*per, per,per);
...
-(void)set_op_focus_sign
{
    if(map_state[focus_x][focus_y]==0)
    {
        [chess_map[focus_x][focus_y] setImage:[UIImage imageNamed:@"hdfocus_aimed_pic"]  forState:UIControlStateNormal];
    }
    else{
